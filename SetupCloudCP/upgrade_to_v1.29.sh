...
# Kubernetes Control Plane Upgrade Script: v1.28 -> v1.29
# Run this script on the control plane node as root or with sudo

echo "=== Upgrading Kubernetes Control Plane: v1.28 → v1.29 ==="

# 1. Backup etcd (skip if etcdctl not available)
echo "[1/6] Backing up etcd..."
mkdir -p /var/backups/kubernetes
...
# 2. Update apt repository to v1.29
echo "[2/6] Updating apt repository to v1.29..."
K8S_VERSION="1.29"
...
DEBIAN_FRONTEND=noninteractive apt-get install -y kubeadm=1.29.15-1.1
...
kubeadm upgrade apply v1.29.15 -y

# 5. Upgrade kubelet and kubectl
echo "[6/6] Upgrading kubelet and kubectl..."
apt-mark unhold kubelet kubectl
DEBIAN_FRONTEND=noninteractive apt-get install -y kubelet=1.29.15-1.1 kubectl=1.29.15-1.1
...
echo ""
echo "=== Upgrade to v1.29 Complete ==="
...
echo ""
echo "✓ Control plane successfully upgraded to v1.29.15"
